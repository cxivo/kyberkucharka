...
Pre ostatné tabuľky tento prístup pochopiteľne nezvolím - recepty budú často duplicitné (dokonca je vítané robiť kópie existujúcich receptov s drobnými úpravami), takže nútiť užívateľov vymýšľať jedinečný názov nie je produktívne (hoci som si istí, že ľudia čo píšu názvy typu _"Úžasný jablkový koláč mojej starej mamy z Kysuckého Nového Mesta, po ktorom sa vám budú zbiehať slinkya a na ktorý budú vaši hostia ešte roky spomínať"_ by to prežili). Rozmýšľal som, že zavediem tento princíp "názov = id" aj pre ingrediencie, ale keďže plánujem povoliť užívateľom vytvárať nové, nemôžem im veriť, že vyberú dobrý názov (a nechcem im tiež vysvetľovať, prečo musia vypĺňať samostatne položky "name" a "display name" alebo také niečo).
